...
## 8. 哈希去重 (LSH) 在合成数据上的失效
* **现象**：使用 MinHash + LSH 算法对 1550 条大模型生成的 QA 进行去重，发现剔除率仅为 0.5%（7条）。
* **根因**：LSH 依赖 N-gram 的字面重合度（Jaccard 相似度）。大模型是“改写大师”，在提问同一个知识点时会使用完全不同的句式和高级词汇，导致字面重合度极低，无法触发哈希桶碰撞。
* **对策**：放弃基于字面的哈希去重，改用基于 Embedding 向量的语义去重（Semantic Deduplication）。在 CPU 上使用轻量级模型 `all-MiniLM-L6-v2` 成功剔除 5.9% 的隐性冗余数据。

## 9. 异步流水线中的“字段丢失”与“结构错位”
* **现象**：在执行格式转换或裁判打分时，频繁出现 `KeyError: 'content'` 或转换结果为 0 条。
* **根因**：数据在经过 `Deduplicator` 时被“拍平（Flatten）”了，且为了节省存储，中间态 JSONL 移除了原始 `content`。
* **对策**：引入 `chunk_id` 作为全局索引锚点。在 `LLM-as-a-Judge` 阶段，通过 `chunk_id` 实时回溯原始 `chunks.jsonl` 获取上下文，实现“轻量化存储 + 动态关联”。

## 10. 大模型合成数据的“语义漂移”
* **现象**：字面哈希去重（LSH）几乎失效，无法识别换汤不换药的重复提问。
* **根因**：LLM 具备极强的改写能力，同一个知识点会变换 10 种句式提问。
* **对策**：必须坚持使用 Embedding 语义去重，且必须在去重前按 `difficulty` 排序，否则算法可能会随机保留一个“水题”而删掉“硬核题”。

## 11. 状态追踪不全导致的重复重试
* **现象**：重启脚本时，逻辑上已处理的空数据被重新拉起。
* **根因**：仅记录了有产出的 ID，未记录“经判定无产出”的 ID。
* **对策**：实现“全量状态追踪”，无论生成结果是否为空，只要请求成功即标记为已处理。

## 12. 显存瓶颈下的模型选择 (RTX 5060 8G)
* **风险**：7B 模型在 8G 显存上极易 OOM。
* **对策**：Phase 5 必须强制使用 QLoRA (4-bit) 并在训练时开启 Gradient Checkpointing，同时严格限制训练时的 Max Sequence Length。
