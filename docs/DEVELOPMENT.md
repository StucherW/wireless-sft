...
**目标**：剔除幻觉与低质量数据，完成语义提纯
**当前状态：✅ 已完成并验证**

### Sub-task 4.1：规则过滤与语义去重
- [√] 规则清洗：完成空列表过滤与 LaTeX 奇偶校验。
- [√] 语义提纯：实现基于 Embedding 的去重，冗余率 5.9%，保留 1458 条 Unique QA。
- [√] 技术调研：对比 LSH 哈希去重，确认语义去重在合成数据场景的优越性。
