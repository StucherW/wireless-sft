...
## 6. 质量控制与去重 (Quality Control) 工作流
本模块负责对大模型生成的 QA 数据进行清洗、去重与格式对齐，确保 SFT 数据集的高纯度。

1. **规则过滤 (Rule-based Cleaning)​**：
   - 剔除生成内容为空（`[]`）的无效 Chunk。
   - 校验 LaTeX 完整性（检查 `$` 符号是否成对），防止破损公式污染模型数学能力。
2. **高维语义去重 (Semantic Deduplication)​**：
   - 将 QA 按难度（Difficulty）降序排序，确保保留的始终是最硬核的版本。
   - 调用 `sentence-transformers` 提取问题向量。
   - 计算余弦相似度矩阵，设置阈值（如 0.85），利用贪心算法剔除大模型生成的“改写版”冗余问题。
   - *(注：曾尝试使用 MinHash+LSH 哈希去重，但因大模型生成的句式差异过大导致碰撞率极低，故废弃该方案)*。
